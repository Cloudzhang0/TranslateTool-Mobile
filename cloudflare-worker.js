...
// MD5 实现（Cloudflare Worker 不支持 Node.js crypto）
function md5(string) {
...
async function handleTranslate(request, env) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: '翻译请求失败: ' + error.message }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 路由：/translate
    if (url.pathname === '/translate' || url.pathname === '/') {
      return handleTranslate(request, env);
    }

    // 404
    return new Response(
      JSON.stringify({ error: '未找到该接口' }),
      {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
